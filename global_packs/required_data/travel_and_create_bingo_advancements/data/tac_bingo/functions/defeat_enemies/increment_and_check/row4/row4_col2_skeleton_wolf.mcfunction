...
scoreboard players add @s tac_kills_earthtojavamobs_skeleton_wolf 1

# Tell the player how many of that mob they have killed, and remind them of the target number
tellraw @s [{"text":"You have defeated "},{"score":{"objective":"tac_kills_earthtojavamobs_skeleton_wolf","name":"@s"}},{"text":"/100 Skeleton Wolves."}]

# If not at the target number, revoke the advancement that triggered this so that this will run again
execute unless score @s tac_kills_earthtojavamobs_skeleton_wolf matches 100.. run advancement revoke @s only tac_bingo:defeat_enemies/row4/row4_col2_skeleton_wolf

# If at the target number, tell the player that they got the advancement
# This makes up for announce_to_chat being false
execute if score @s tac_kills_earthtojavamobs_skeleton_wolf matches 100.. run tellraw @s [{"text":"You've earned the advancement for defeating 100 Skeleton Wolves! Ring the bell if you have a bingo!"}]
