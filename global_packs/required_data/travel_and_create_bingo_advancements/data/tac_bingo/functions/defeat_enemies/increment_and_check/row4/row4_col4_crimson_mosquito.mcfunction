...
scoreboard players add @s tac_kills_alexsmobs_crimson_mosquito 1

# Tell the player how many of that mob they have killed, and remind them of the target number
tellraw @s [{"text":"You have defeated "},{"score":{"objective":"tac_kills_alexsmobs_crimson_mosquito","name":"@s"}},{"text":"/100 Crimson Mosquitos."}]

# If not at the target number, revoke the advancement that triggered this so that this will run again
execute unless score @s tac_kills_alexsmobs_crimson_mosquito matches 100.. run advancement revoke @s only tac_bingo:defeat_enemies/row4/row4_col4_crimson_mosquito

# If at the target number, tell the player that they got the advancement
# This makes up for announce_to_chat being false
execute if score @s tac_kills_alexsmobs_crimson_mosquito matches 100.. run tellraw @s [{"text":"You've earned the advancement for defeating 100 Crimson Mosquitos! Ring the bell if you have a bingo!"}]
