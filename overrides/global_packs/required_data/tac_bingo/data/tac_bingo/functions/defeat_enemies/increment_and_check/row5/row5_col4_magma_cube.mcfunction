...
scoreboard players add @s tac_kills_minecraft_magma_cube 1

# Tell the player how many of that mob they have killed, and remind them of the target number
tellraw @s [{"text":"You have defeated "},{"score":{"objective":"tac_kills_minecraft_magma_cube","name":"@s"}},{"text":"/100 Magma Cubes."}]

# If not at the target number, revoke the advancement that triggered this so that this will run again
execute unless score @s tac_kills_minecraft_magma_cube matches 100.. run advancement revoke @s only tac_bingo:defeat_enemies/row5/row5_col4_magma_cube

# If at the target number, tell the player that they got the advancement
# This makes up for announce_to_chat being false
execute if score @s tac_kills_minecraft_magma_cube matches 100.. run tellraw @s [{"text":"You've earned the advancement for defeating 100 Magma Cubes! Ring the bell if you have a bingo!"}]
