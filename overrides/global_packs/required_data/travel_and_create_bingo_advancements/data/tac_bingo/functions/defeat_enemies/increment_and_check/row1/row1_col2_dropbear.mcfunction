...
scoreboard players add @s tac_kills_alexsmobs_dropbear 1

# Tell the player how many of that mob they have killed, and remind them of the target number
tellraw @s [{"text":"You have defeated "},{"score":{"objective":"tac_kills_alexsmobs_dropbear","name":"@s"}},{"text":"/100 Dropbears."}]

# If not at the target number, revoke the advancement that triggered this so that this will run again
execute unless score @s tac_kills_alexsmobs_dropbear matches 100.. run advancement revoke @s only tac_bingo:defeat_enemies/row1/row1_col2_dropbear

# If at the target number, tell the player that they got the advancement
# This makes up for announce_to_chat being false
execute if score @s tac_kills_alexsmobs_dropbear matches 100.. run tellraw @s [{"text":"You've earned the advancement for defeating 100 Dropbears! Ring the bell if you have a bingo!"}]
